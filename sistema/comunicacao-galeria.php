...
<div id="conteudo">
	<div class="topo">
		<h1>COMUNICAÇÃO GALERIA</h1>
        <div>
			<? if($acao != 'remover'){ ?>
			<a href="comunicacao-galeria.php?acao=adicionar" class="adicionar">adicionar imagens</a>
	...
	<? if($acao == 'adicionar'){ ?>
		...
		<form id="fileupload-comunicacao" action="" method="POST" enctype="multipart/form-data">
	...
	elseif($acao == ''){
		...
		$query = mysql_query("select * from ".COMUNICACAO_GAL." order by posicao desc");
		...
		<? } else { ?>
            ...
            <div class="listagem">
                <div id="sortable" class="img">
                    <input type="hidden" name="tabela" id="tabela" value="<?=COMUNICACAO_GAL?>" />
                    ...
                    while($rs = mysql_fetch_assoc($query)){
                        ...
                        <div class="caixa-instituto-img posiciona" data-id="<?=$rs['idimg']?>">
                            ...
                            <span class="acoes">
                                <span class="ativo"><img src="../img/sistema/<?=$ativo?>" longdesc="<?=COMUNICACAO_GAL?>-<?=$rs['idimg']?>-idimg" class="ativo cursor vimg" /></span>
                                ...
                                <span class="remover"><a href="javascript:void(0)" onClick="removerIMGCom(<?=$rs['idimg']?>)"><img src="../img/sistema/ico-remover.gif" alt="remover" border="0" class="vimg" /></a></span>
	...
	elseif($acao == 'editar'){
		...
		if(!$_POST){
			...
			include('form/form-comunicacao-galeria.php');
			echo '</div>';
		} else {
			...
			atualizar(COMUNICACAO_GAL,$dados,"idimg = ".$idimg);
			...
			if($imagem['name'] != ''){
				...
				atualizar(COMUNICACAO_GAL,"imagem = '".$img."',crop = 'N'","idimg = ".$idimg);
				$crop = true;
			}
	...
	elseif($acao == 'crop'){
		if(!$_POST){
			...
			$sql = mysql_query("select * from ".COMUNICACAO_GAL." where crop = 'N'");
			?>
			<div class="conteudo">
		...
		<?
			...
			for($i = 0; $i <= $total; $i++){
				...
				if($img != ''){
					...
					atualizar(COMUNICACAO_GAL,"crop = 'S'","crop = 'N'");
	...
	elseif($acao == 'remover'){
		if(is_numeric($id)){
			...
			$img = mysql_fetch_assoc(mysql_query("select * from ".COMUNICACAO_GAL." where idimg = ".$id));
			...
			deletar(COMUNICACAO_GAL,"where idimg = ".$id);
		}
