...
<div id="conteudo">
	...
	if($acao == ''){
		...
		$query = mysql_query("select * from ".PORTAIS." where tipo = 'CL' order by posicao desc");
		...
		<? } else { ?>
        	<div class="listagem">
				...
				<div id="sortable">
                    ...
                    <input type="hidden" name="tipo" id="tipo" value="CL" />
	...
	elseif($acao == 'adicionar'){
		...
		if(!$_POST){
			...
			include('form/form-portal-cliente.php');
			echo '</div>';
		} else {
			...
			$tipoLink = seguranca($_POST['tipoLink']);
			$ativo = ($ativo == 'S') ? 'S' : 'N';

			$campos = "idioma,tipo,titulo,tag,url,ativo,posicao,tipoLink";
			$valores = "'".$idioma."','".$tipo."','".$titulo."','".$tag."','".$url."','".$ativo."','".$posicao."','".$tipoLink."'";
		...
		if (!$_POST){
			...
			$dados = "idioma = '".$idioma."',tipo = '".$tipo."',titulo = '".$titulo."',tag = '".$tag."',url = '".$url."',ativo = '".$ativo."',posicao = '".$posicao."',tipoLink = '".$tipoLink."'";
