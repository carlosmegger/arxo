...
#titulo
$sql = mysql_fetch_assoc(mysql_query("select * from ".REVISTAS." where idrevista = '".$idrevista."'"));
$tit = 'Revistas Galeria &raquo; '.$sql['titulo'];
?>
<div id="conteudo">
	<div class="topo">
		<h1><?=$tit?></h1>
        <div>
			<? if($acao != 'remover'){ ?>
			<a href="revistas-imgs.php?acao=adicionar&idrevista=<?=$idrevista?>" class="adicionar">adicionar imagens</a>
            <a href="javascript:history.back()" class="voltar">&laquo; voltar</a>
            <? } ?>
        </div>
	</div>

	<? if($acao == 'adicionar' && $idrevista != ''){ ?>

		<p>
		1) Selecione as imagens desejadas - <span style="color:#d00">1500px de largura x 2000px de altura</span>;<br />
		2) Clique em enviar imagens para efetuar o envio;<br />
		3) Após o envio você terá acesso a listagem com as imagens cadastradas.
		</p>
		<br />

		<form id="fileupload-revistas" action="" method="POST" enctype="multipart/form-data">
	...
	elseif($acao == 'listar' && $idrevista != ''){

		$query = mysql_query("select * from ".REVISTAS_IMGS." where idrevista = '".$idrevista."' order by posicao desc");
		...
		<div class="formulario">
			<?
            $acao = 'adicionar';
			include('form/form-revistas-imgs.php');
			?>
		</div>
		<hr size="1" />
		<? } ?>
		...
		<div class="listagem">
			<div id="sortable" class="img">
            	<input type="hidden" name="tabela" id="tabela" value="<?=REVISTAS_IMGS?>" />
                <input type="hidden" name="campo" id="campo" value="idimg" />
				<input type="hidden" name="idrevista" id="idrevista" value="<?=$rs['idrevista']?>" />
                
				<?
				while($rs = mysql_fetch_assoc($query)){
					$ativo = ($rs['ativo'] == 'S') ? 'ico-ativo-on.png' : 'ico-ativo-off.png';
					?>
                    <div class="caixa-revistas-img posiciona" data-id="<?=$rs['idimg']?>">
						...
						<span class="acoes">
                        	<span class="ativo"><img src="../img/sistema/<?=$ativo?>" longdesc="<?=REVISTAS_IMGS?>-<?=$rs['idimg']?>-idimg" class="ativo cursor vimg" /></span>
                            <span class="editar"><a href="?acao=editar&id=<?=$rs['idimg']?>&idrevista=<?=$rs['idrevista']?>"><img src="../img/sistema/ico-editar.png" alt="editar" border="0" class="vimg" /></a></span>
                            <span class="remover"><a href="javascript:void(0)" onClick="removerIMGR(<?=$rs['idimg']?>,<?=$idrevista?>)"><img src="../img/sistema/ico-remover.gif" alt="remover" border="0" class="vimg" /></a></span>
	...
	elseif($acao == 'editar' && $idrevista != ''){
		...
		if(!$_POST){
			...
			$idrevista = seguranca($_POST['idrevista']);
			...
			if($imagem['name'] != ''){
				...
				*/
				...
				atualizar(REVISTAS_IMGS,"imagem = '".$img."',crop = 'N'","idimg = ".$id);
				#$crop = true;
			}

			if($crop){
				echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=?acao=crop&id=$id&idrevista=$idrevista'>";
			} else {
				echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=?atualizado=true&acao=listar&idrevista=$idrevista'>";
			}
		}
	}
	/*
	elseif($acao == 'crop'){
		if(!$_POST){
			...
			$sql = mysql_query("select * from ".REVISTAS_IMGS." where idrevista = '".$idrevista."' and crop = 'N'");
			?>
			<div class="conteudo">
		...
		<?
			...
			for($i = 0; $i <= $total; $i++){
				...
				if($img != ''){
					...
					atualizar(REVISTAS_IMGS,"crop = 'S'","crop = 'N'");
					if(is_file($path_temp.$img)) unlink($path_temp.$img);
				}
			}

			echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=?acao=listar&idrevista=$idrevista'>";
		}
	}
	*/
		...
		if(is_numeric($id)){
			...
			$img = mysql_fetch_assoc(mysql_query("select * from ".REVISTAS_IMGS." where idimg = ".$id));
			...
			deletar(REVISTAS_IMGS,"where idimg = ".$id);
		}

		echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=revistas-imgs.php?acao=listar&idrevista=$idrevista'>";
	}
	?>
</div>
