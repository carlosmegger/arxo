...
#titulo
$sql = mysql_fetch_assoc(mysql_query("select * from ".CATEGORIAS." where idcategoria = '".$idcategoria."'"));
$tit = 'Categoria &raquo; '.$sql['titulo'];
?>
<div id="conteudo">
	<div class="topo">
		<h1><?=$tit?></h1>
		<div>
			<? if($acao != 'remover'){ ?>
			<a href="categorias-imgs.php?acao=adicionar&idcategoria=<?=$idcategoria?>&tipo=<?=$tipo?>" class="adicionar">adicionar imagens</a>
			<a href="javascript:history.back()" class="voltar">&laquo; voltar</a>
			<? } ?>
		</div>
	</div>

	<? if($acao == 'adicionar' && $idcategoria != ''){ ?>
		...
		<form id="fileupload-categorias" action="" method="POST" enctype="multipart/form-data">
	...
	elseif($acao == 'listar' && $idcategoria != ''){

		$query = mysql_query("select * from ".CATEGORIAS_IMGS." where idcategoria = '".$idcategoria."' order by posicao desc");
		...
		<div class="formulario">
			<?
            $acao = 'adicionar';
			include('form/form-categorias-imgs.php');
			?>
		</div>
		<hr size="1" />
		<? } ?>
		...
		<div class="listagem">
			<div id="sortable" class="img">
            	<input type="hidden" name="tabela" id="tabela" value="<?=CATEGORIAS_IMGS?>" />
                <input type="hidden" name="campo" id="campo" value="idimg" />
				<input type="hidden" name="idcategoria" id="idcategoria" value="<?=$rs['idcategoria']?>" />
				...
				while($rs = mysql_fetch_assoc($query)){
                    ...
                    <div class="caixa-cat-img posiciona" data-id="<?=$rs['idimg']?>">
						...
						<span class="acoes">
                        	<span class="ativo"><img src="../img/sistema/<?=$ativo?>" longdesc="<?=CATEGORIAS_IMGS?>-<?=$rs['idimg']?>-idimg" class="ativo cursor vimg" /></span>
                            <span class="editar"><a href="?acao=editar&id=<?=$rs['idimg']?>&idcategoria=<?=$rs['idcategoria']?>&tipo=<?=$tipo?>"><img src="../img/sistema/ico-editar.png" alt="editar" border="0" class="vimg" /></a></span>
							<span class="remover"><a href="javascript:void(0)" onClick="removerIMGC(<?=$rs['idimg']?>,<?=$idcategoria?>,'<?=$tipo?>')"><img src="../img/sistema/ico-remover.gif" alt="remover" border="0" class="vimg" /></a></span>
	...
	elseif($acao == 'editar' && $idcategoria != ''){
		...
		if(!$_POST){
			...
			atualizar(CATEGORIAS_IMGS,"legendaBR = '".$legendaBR."',legendaEN = '".$legendaEN."',legendaES = '".$legendaES."'","idimg = ".$id);
			...
			if($imagem['name'] != ''){
				...
				atualizar(CATEGORIAS_IMGS,"imagem = '".$img."',crop = 'N'","idimg = ".$id);
				$crop = true;
			}

			if($crop){
				echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=?acao=crop&id=$id&idcategoria=$idcategoria&tipo=$tipo'>";
			} else {
				echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=?atualizado=true&acao=listar&idcategoria=$idcategoria&tipo=$tipo'>";
			}
		}
	}
	elseif($acao == 'crop'){
		if(!$_POST){
			...
			$sql = mysql_query("select * from ".CATEGORIAS_IMGS." where idcategoria = '".$idcategoria."' and crop = 'N'");
			?>
			<div class="conteudo">
				<form name="" id="" action="?acao=crop&tipo=<?=$tipo?>" method="post" enctype="multipart/form-data">
		...
		<?
			...
			for($i = 0; $i <= $total; $i++){
				...
				if($img != ''){
					...
					atualizar(CATEGORIAS_IMGS,"legendaBR = '".$legendaBR."',legendaEN = '".$legendaEN."',legendaES = '".$legendaES."',crop = 'S'","crop = 'N'");
					if(is_file($path_temp.$img)) unlink($path_temp.$img);
				}
			}

			echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=?acao=listar&idcategoria=$idcategoria&tipo=$tipo'>";
		}
	}
	else if($acao == 'remover'){
		if(is_numeric($id)){

			$img = mysql_fetch_assoc(mysql_query("select * from ".CATEGORIAS_IMGS." where idimg = ".$id));
			...
			deletar(CATEGORIAS_IMGS,"where idimg = ".$id);
		}

		echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=categorias-imgs.php?acao=listar&idcategoria=$idcategoria&tipo=$tipo'>";
	}
	?>
</div>
