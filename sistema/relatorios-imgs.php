...
#titulo
$sql = mysql_fetch_assoc(mysql_query("select * from ".RELATORIOS." where idrelatorio = '".$idrelatorio."'"));
$tit = 'Relatório &raquo; '.$sql['titulo'];
?>
<div id="conteudo">
	<div class="topo">
		<h1><?=$tit?></h1>
        <div>
			<? if($acao != 'remover'){ ?>
			<a href="relatorios-imgs.php?acao=adicionar&idrelatorio=<?=$idrelatorio?>" class="adicionar">adicionar imagens</a>
			<a href="javascript:history.back()" class="voltar">&laquo; voltar</a>
			<? } ?>
        </div>
	</div>

	<? if($acao == 'adicionar' && $idrelatorio != ''){ ?>
		...
		<form id="fileupload-relatorios" action="" method="POST" enctype="multipart/form-data">
	...
	elseif($acao == 'listar' && $idrelatorio != ''){

		$query = mysql_query("select * from ".RELATORIOS_IMGS." where idrelatorio = '".$idrelatorio."' order by posicao desc");
		...
		<div class="formulario">
			<?
            $acao = 'adicionar';
            include('form/form-relatorios-imgs.php');
            ?>
		</div>
		<hr size="1" />
		<? } ?>
		...
		<div class="listagem">
			<div id="sortable" class="img">
            	<input type="hidden" name="tabela" id="tabela" value="<?=RELATORIOS_IMGS?>" />
                <input type="hidden" name="campo" id="campo" value="idimg" />
				<input type="hidden" name="idrelatorio" id="idrelatorio" value="<?=$rs['idrelatorio']?>" />
				...
				while($rs = mysql_fetch_assoc($query)){
                    ...
                    <div class="caixa-cat-img posiciona" data-id="<?=$rs['idimg']?>">
						...
						<span class="acoes">
                        	<span class="ativo"><img src="../img/sistema/<?=$ativo?>" longdesc="<?=RELATORIOS_IMGS?>-<?=$rs['idimg']?>-idimg" class="ativo cursor vimg" /></span>
                            <span class="editar"><a href="?acao=editar&id=<?=$rs['idimg']?>&idrelatorio=<?=$rs['idrelatorio']?>"><img src="../img/sistema/ico-editar.png" alt="editar" border="0" class="vimg" /></a></span>
                            <span class="remover"><a href="javascript:void(0)" onClick="removerIMGP(<?=$rs['idimg']?>,<?=$idrelatorio?>)"><img src="../img/sistema/ico-remover.gif" alt="remover" border="0" class="vimg" /></a></span>
	...
	elseif($acao == 'editar' && $idrelatorio != ''){
		...
		if(!$_POST){
			...
			$idrelatorio = seguranca($_POST['idrelatorio']);
			...
			atualizar(RELATORIOS_IMGS,"legendaBR = '".$legendaBR."',legendaEN = '".$legendaEN."',legendaES = '".$legendaES."'","idimg = ".$id);
			...
			if($imagem['name'] != ''){
				...
				atualizar(RELATORIOS_IMGS,"imagem = '".$img."',crop = 'N'","idimg = ".$id);
				$crop = true;
			}

			if($crop){
				echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=?acao=crop&id=$id&idrelatorio=$idrelatorio'>";
			} else {
				echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=?atualizado=true&acao=listar&idrelatorio=$idrelatorio'>";
			}
		}
	}
	elseif($acao == 'crop'){
		if(!$_POST){
			...
			$sql = mysql_query("select * from ".RELATORIOS_IMGS." where idrelatorio = '".$idrelatorio."' and crop = 'N'");
			?>
			<div class="conteudo">
		...
		<?
			...
			for($i = 0; $i <= $total; $i++){
				...
				if($img != ''){
					...
					atualizar(RELATORIOS_IMGS,"legendaBR = '".$legendaBR."',legendaEN = '".$legendaEN."',legendaES = '".$legendaES."',crop = 'S'","crop = 'N'");
					if(is_file($path_temp.$img)) unlink($path_temp.$img);
				}
			}

			echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=?acao=listar&idrelatorio=$idrelatorio'>";
		}
	}
	else if($acao == 'remover'){
		if(is_numeric($id)){

			$img = mysql_fetch_assoc(mysql_query("select * from ".RELATORIOS_IMGS." where idimg = ".$id));
			...
			deletar(RELATORIOS_IMGS,"where idimg = ".$id);
		}

		echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=relatorios-imgs.php?acao=listar&idrelatorio=$idrelatorio'>";
	}
	?>
</div>
