...
if($acao == 'editar'){
	if(is_numeric($id)){
		$sql = selecionar("",RELATORIOS_IMGS,"where idimg = ".$id);
...
?>
<form name="form-relatorios-imgs" id="form-relatorios-imgs" class="<?=$acao?>" method="post" enctype="multipart/form-data" action="<?=($acao == 'adicionar') ? '?acao=adicionar' : '' ?>">
	...
	<? if($acao == 'editar'){ ?>
        ...
        <span>
			...
			<div class="small" style="margin:4px 0">(Tamanho ideal: 600px de largura x 800px de altura)</div>
		</span>
