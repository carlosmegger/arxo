...
<form name="form-portais" id="form-portais" method="post" enctype="multipart/form-data" action="">
	...
	<span>
		...
		<? if($acao == 'editar' && $icone != ''){ ?><a href="<?=$path.$icone?>" class="fancybox"><img src="../img/sistema/ico-zoom.png" class="vimg" /> visualizar</a><? } ?>
		<div class="small">(Tamanho ideal: 180px de largura x 180px de altura)</div>
	</span>
	<span>
		<label><input type="radio" name="tipoLink" class="tipoLink" value="link" <?=$tipoLink=='link'?'checked':''?> /> Link</label>
		<label><input type="radio" name="tipoLink" class="tipoLink" value="download" <?=$tipoLink=='download'?'checked':''?> /> Arquivo</label>
	</span>
	<span id="bloco-link" <?=$tipoLink=='download'?'style="display:none"':''?>>
