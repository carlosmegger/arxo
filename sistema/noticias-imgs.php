...
#titulo
$sql = mysql_fetch_assoc(mysql_query("select * from ".NOTICIAS." where idnoticia = '".$idnoticia."'"));
$tit = 'Notícia &raquo; '.$sql['titulo'];
?>
<div id="conteudo">
	<div class="topo">
		<h1><?=$tit?></h1>
        <div>
			<? if($acao != 'remover'){ ?>
			<a href="noticias-imgs.php?acao=adicionar&idnoticia=<?=$idnoticia?>" class="adicionar">adicionar imagens</a>
			<a href="javascript:history.back()" class="voltar">&laquo; voltar</a>
			<? } ?>
        </div>
	</div>

	<? if($acao == 'adicionar' && $idnoticia != ''){ ?>
		...
		<form id="fileupload-noticias" action="" method="POST" enctype="multipart/form-data">
	...
	elseif($acao == 'listar' && $idnoticia != ''){

		$query = mysql_query("select * from ".NOTICIAS_IMGS." where idnoticia = '".$idnoticia."' order by posicao desc");
		...
		<div class="formulario">
			<?
            $acao = 'adicionar';
			include('form/form-noticias-imgs.php');
			?>
		</div>
		<hr size="1" />
		<? } ?>
		...
		<div class="listagem">
			<div id="sortable">
            	<input type="hidden" name="tabela" id="tabela" value="<?=NOTICIAS_IMGS?>" />
                <input type="hidden" name="campo" id="campo" value="idimg" />
				<input type="hidden" name="idnoticia" id="idnoticia" value="<?=$rs['idnoticia']?>" />
                
				<?
				while($rs = mysql_fetch_assoc($query)){
					$ativo = ($rs['ativo'] == 'S') ? 'ico-ativo-on.png' : 'ico-ativo-off.png';
					?>
                    <div class="caixa-not-img posiciona" data-id="<?=$rs['idimg']?>">
						...
						<span class="acoes">
                        	<span class="ativo"><img src="../img/sistema/<?=$ativo?>" longdesc="<?=NOTICIAS_IMGS?>-<?=$rs['idimg']?>-idimg" class="ativo cursor vimg" /></span>
                            <span class="editar"><a href="?acao=editar&id=<?=$rs['idimg']?>&idnoticia=<?=$rs['idnoticia']?>"><img src="../img/sistema/ico-editar.png" alt="editar" border="0" class="vimg" /></a></span>
                            <span class="remover"><a href="javascript:void(0)" onClick="removerIMGP(<?=$rs['idimg']?>,<?=$idnoticia?>)"><img src="../img/sistema/ico-remover.gif" alt="remover" border="0" class="vimg" /></a></span>
	...
	elseif($acao == 'editar' && $idnoticia != ''){
		...
		if(!$_POST){
			...
			$idnoticia = seguranca($_POST['idnoticia']);
			...
			atualizar(NOTICIAS_IMGS,"legendaBR = '".$legendaBR."',legendaEN = '".$legendaEN."',legendaES = '".$legendaES."'","idimg = ".$id);
			...
			if($imagem['name'] != ''){
				...
				atualizar(NOTICIAS_IMGS,"imagem = '".$img."',crop = 'N'","idimg = ".$id);
				$crop = true;
			}

			if($crop){
				echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=?acao=crop&id=$id&idnoticia=$idnoticia'>";
			} else {
				echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=?atualizado=true&acao=listar&idnoticia=$idnoticia'>";
			}
		}
	}
	elseif($acao == 'crop'){
		if(!$_POST){
			...
			$sql = mysql_query("select * from ".NOTICIAS_IMGS." where idnoticia = '".$idnoticia."' and crop = 'N'");
			?>
			<div class="conteudo">
		...
		<?
			...
			for($i = 0; $i <= $total; $i++){
				...
				if($img != ''){
					...
					atualizar(NOTICIAS_IMGS,"legendaBR = '".$legendaBR."',legendaEN = '".$legendaEN."',legendaES = '".$legendaES."',crop = 'S'","crop = 'N'");
					if(is_file($path_temp.$img)) unlink($path_temp.$img);
				}
			}

			echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=?acao=listar&idnoticia=$idnoticia'>";
		}
	}
	else if($acao == 'remover'){
		if(is_numeric($id)){

			$img = mysql_fetch_assoc(mysql_query("select * from ".NOTICIAS_IMGS." where idimg = ".$id));
			...
			deletar(NOTICIAS_IMGS,"where idimg = ".$id);
		}

		echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=noticias-imgs.php?acao=listar&idnoticia=$idnoticia'>";
	}
	?>
</div>
